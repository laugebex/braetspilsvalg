...
const {
  parseVotes, resultsForPoll, missingVoters, votedNames, topIds, canonicalVoter
} = require('./shared');
...
async function loadElection(poll) {
  const mainVotes = parseVotes(await command(['HGETALL', votesKey(poll.id)])).filter((v) => canonicalVoter(poll, v.name));
