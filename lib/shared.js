...
function isSupersededMainVote(poll, vote) {
  if (!poll || !vote?.name) return false;
  const cutoff = poll.voteResetCutoffs?.[voterKey(vote.name)];
  if (!cutoff) return false;
  const voteTime = Date.parse(vote.createdAt || vote.updatedAt || '');
  const cutoffTime = Date.parse(cutoff);
  return Number.isFinite(voteTime) && Number.isFinite(cutoffTime) && voteTime <= cutoffTime;
}
...
module.exports = {
  ...
  isSupersededMainVote, parseVotes, votedNames, missingVoters, resultsForPoll, topIds
};
